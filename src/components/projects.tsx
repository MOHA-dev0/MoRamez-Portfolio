...
function Projects({ projects }: ProjectsProps) {
  ...
  const applyFilters = (data: Project[], currentFilterValue: string) => {
    // إذا كان الفلتر فارغ أو "all" نعيد كل البيانات
    if (!currentFilterValue || currentFilterValue.toLowerCase() === "all") {
      return data;
    }

    // تنظيف قيمة الفلتر
    const normalizedFilter = currentFilterValue.toLowerCase().trim();

    // فلترة المشاريع
    return data.filter((project) =>
      project.techStack.some((tech) => {
        // تنظيف كل تقنية في المصفوفة
        const normalizedTech = tech.toLowerCase().trim();
        return normalizedTech === normalizedFilter;
      })
    );
  };
  return (
      ...
      <div className="md:py-8 relative">
        ...
        <div className="overflow-hidden max-md:hidden">
          <motion.p
            ...
            className="absolute text-white/50 line-clamp-1"
          >
            {description}
          </motion.p>
        </div>
